...
module('userMap').
  component('userMap', {
    ...
    controller: function madcapController($scope, NgMap) {
		 ...
		 */
		$scope.show10markers = function()	{
			var strUser = document.getElementById("chosen_user").options[document.getElementById("chosen_user").selectedIndex].value;
        	gapi.client.analysisEndpoint.getLastFor({'amount' : 10, 'user' : strUser}).execute(function(resp) {
        	   	if(resp.entries !== null)	{
        	   		// Filler data, because there is no location data in the Cloud storage yet
		...
		function showOnMap(entries)	{
			...
			function recursiveMarkerAdd(interval, entries)	{
				var i = interval - 1;
				markers[i] = new google.maps.Marker({
					title: "Time: " + getTimeFromUnix(entries.timestamp)
		    	});
				var location = getLocationData(entries[i]);
				markers[i].setPosition(new google.maps.LatLng(location[0], location[1]))
				bounds.extend(markers[i].getPosition());
				var cache = markers[i];
				// Requests the shown Google Map
				NgMap.getMap().then(function(map){
					...
					// Call to add the next marker, if there is a next marker to be set
					if(--interval !== 0)	{
						recursiveMarkerAdd(interval, entries);
					}
				});
			}
			
			recursiveMarkerAdd(entries.length, entries);
		}
		
		/**
		 * TO BE IMPLEMENTED. Currently generates a random marker nearby the CESE.
		...
		function getLocationData(entry)	{
			var returned = [];
			returned[0] = Math.random() * (38.98-38.96) + 38.96; 
			returned[1] = (Math.random() * (76.93-76.91) + 76.91) * -1; 
			return returned;
		}
